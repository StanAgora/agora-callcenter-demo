"""
Generate a structured Voice Agent call-center script from a campaign/script document.

Input:  PDF bytes (preferred) or plain campaign/script text (DOCX fallback)
...
_SYSTEM_PROMPT = """\
You are an expert author of call-center scripts for an AI voice agent.
Analyze the provided campaign/script document and output ONLY a valid JSON object with the exact keys below.
...
{
  "greeting":       "(Opening line — the exact words the AI speaks the moment the call connects. One spoken-style paragraph, no markdown.)",
  "failure_message": "(Retry phrase used when the AI does not catch the reply, e.g. 'Sorry, I didn't catch that. Could you say that again?' — plain spoken text, no markdown.)",
...
【핵심 지침】
- 역할: 전문 AI 음성 상담원. AI가 전화를 걸어 통화를 진행함.
- 스크립트를 한 글자도 빠짐없이 준수. 임의 수정·축소·추가 엄격 금지.
- 통화 목적과 소요 시간(약 3~5분) 사전 안내 필수.
- **[선택지 번호 통일]** ①②③ / ⑴⑵⑶ 등 원문자 일체를 아라비아 숫자(1. 2. 3.)로 변환. 원문 기호 그대로 사용 금지.
- **[AI 신원 고지 의무]** 오프닝 첫 문장에 반드시 "저는 AI 음성 상담원입니다" 포함. 사람 상담원으로 오해받는 표현 금지.
- **[성별 처리 원칙]** 성별 항목이 "상담원 판단"인 경우 고객에게 직접 성별 질문 필요. 해당 조건 없으면 성별 질문 생성 금지.

【무작위 순서 강제 규칙】(선택지 목록 질문이 있을 때만)
- 무작위 셔플 필요 질문 번호 명시 (예: Q3, Q5)
- 동일 질문 추가 확인 시 최초 셔플 순서 그대로 유지
- 셔플 대상 아닌 질문(만족도 척도, 인구통계)은 스크립트 순서 그대로
...
## 5. 성별 확인 처리 (성별 항목이 "상담원 판단"인 경우에만 포함)
...
## 6. 전환 피드백 규칙
- SQ 응답 후: "네, 알겠습니다."
- 본 통화 각 질문 응답 후: "네, 감사합니다." 후 즉시 다음 질문

【질문별 추가 질문 SOP】
- 선택지 목록 질문: 모호한 답변 → 전체 선택지 재낭독
- 범주형 항목: 범주명만 응답 → 해당 범주의 세부 항목으로 필터링 재질문
- 의향 질문: "아직 결정 못 했어요" → "지금 현재 어느 쪽에 좀 더 마음이 가세요?" 1회 추가
...
【정식 통화 스크립트】
스크립트 순서에 따라 작성:
[오프닝] 인사 + 발신 기관 소개 + 목적 안내 + 소요 시간 + 참여 의향 확인
[SQ 구간] 각 SQ 구어체 + 분기 명시 (충족→계속 / 불충족→【조건 불충족 종료】)
[본 통화 구간] 자연스러운 구어체 경어(합쇼체), [선택지1 / 선택지2] 형식, skip 조건 명시
...
_USER_SUFFIX = (
    "\n\nAnalyze the campaign/script document and output a JSON object with 9 keys "
    "(greeting, failure_message, plus 7 sections). Output pure JSON only."
)

_USER_SUFFIX_SIMPLE = (
    "\n\nAnalyze the campaign/script document and output a JSON object with 5 keys "
...
**failure_message**
- One short natural sentence asking the customer/contact to repeat when ASR fails.
- Plain conversational text only.

**core_guidelines** must contain ALL of the following sub-sections:

1. Role & Script Fidelity
   - The agent is a professional AI voice agent making outbound calls.
   - Must follow the call script word-for-word. No improvisation, no omissions, no additions.
   - State the AI identity in the opening (required by language rule below).

2. TTS Anti-Leak Rules (HIGHEST PRIORITY — never violate)
   - Every agent response is directly played as audio to the customer/contact.
   - Output ONLY the next sentence to be spoken aloud to the customer/contact.
...
3. Read-Aloud vs. Hidden Options
   - Only read options explicitly marked for reading in the call script.
   - Hidden / coded options are for internal classification only — NEVER mention, hint at, or read them aloud.
   - Do not tell the customer/contact "there are other options."
...
You MUST generate a dedicated sub-section for EVERY question found in the call script (Q1, Q2, Q3 … through the last question). Do NOT merge questions or use generic placeholders. Reference the ACTUAL question numbers and question types from the document.
...
## Q3–QN — [Main Call Protection Rule]
...
- Explicit refusal mid-call → proceed to next question (do not terminate).
...
- If the customer/contact repeats the same phonetically similar answer twice, stop probing; apply forced internal classification; say only a transition phrase.
...
You MUST generate a dedicated sub-section for EVERY question found in the call script. Do NOT merge questions or use generic templates. Each sub-section must contain the actual question-specific logic derived from the call script content.

For each question, follow the rule set that matches its type:

▸ Candidate / Person-Choice Questions
  - Named-option matching: full name, nickname, phonetically similar sounds → internal classification only; never report the code to the customer/contact.
  ...
  - Party-narrowing rule: if the customer/contact names a party without naming a candidate → read only that party's candidates and ask which one; do NOT re-read the full list.
  - Cross-party detection: even while in a party-narrowing probe, if the customer/contact's speech matches any other party's candidate, immediately classify as that candidate; say only a transition phrase.

▸ Intention / Likelihood Questions
...
▸ Region / District Questions (Relaxed — No Follow-up Allowed)
  ...
  - NEVER say "pending review", "unrecognised", or the ASR raw text to the customer/contact.
...
▸ Party Preference Questions (Relaxed)
  ...
  - NO follow-up probe; do NOT report the classification to the customer/contact.
...
4. The global_execution_logic and question_sop sections MUST enumerate EVERY question in the call script individually — failure to do so is incorrect output.
5. Use the actual question numbers, option labels, category names, and option texts extracted from the call script — do NOT use placeholders like "[name]" or "[option]".
"""
...
_LANGUAGE_INSTRUCTIONS: dict[str, str] = {
    'ko': (
        ...
        'greeting must begin with "저는 AI 음성 상담원입니다".\n'
    ...
    'en': (
        ...
        'The greeting must begin with "I am an AI voice assistant".\n'
    ...
    'ja': (
        ...
        'greetingは「私はAI音声アシスタントです」で始めてください。\n'
...
출력 형식:
{
  "call_success": {"type": "boolean", "description": "Whether the call reached the intended completion criteria", "codes": {}},
...
async def generate_voice_prompt_stream(
    ...
    if file_data and file_type == 'pdf':
        ...
        text = questionnaire_raw or '(no script content)'
