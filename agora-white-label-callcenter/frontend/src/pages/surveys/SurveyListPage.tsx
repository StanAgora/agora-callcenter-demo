    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      {viewMode === 'list' && (
        <div className="grid gap-4">
          {surveys.map(survey => (
            <div key={survey.id} className="bg-white border border-gray-100 rounded-xl p-5 hover:border-primary-200 transition-colors shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-gray-400 hover:bg-primary-50 hover:text-primary-600 transition-colors disabled:opacity-40"
                  >
      ...
      {duplicateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setDuplicateModal(null)}>
          <div className="bg-white border border-gray-100 rounded-xl shadow-sm w-full max-w-sm mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <input
              type="text"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white mb-4"
              value={duplicateName}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg text-sm font-medium bg-primary-600 text-white hover:bg-primary-700 transition-colors disabled:opacity-40"
              >
      ...
      {viewMode === 'grid' && (
        <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
          {surveys.map(survey => (
            <div key={survey.id} className="bg-white border border-gray-100 rounded-xl p-4 hover:border-primary-200 transition-colors shadow-sm flex flex-col gap-3">
              {/* Top: badges + actions */}
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    ...
                    disabled={!!duplicatingId}
                    title={t('survey_list.btn_duplicate')}
                    className="inline-flex items-center justify-center w-7 h-7 rounded-lg text-gray-300 hover:bg-primary-50 hover:text-primary-600 transition-colors disabled:opacity-40"
                  >
              ...
              <div className="flex items-center gap-1.5 pt-1 border-t border-gray-100">
                <Link
                  to={`/surveys/${survey.id}/prompt`}
                  className="flex-1 inline-flex items-center justify-center gap-1 py-1.5 rounded-lg text-xs font-medium bg-primary-50 text-primary-600 hover:bg-primary-100 transition-colors"
                >
                  <Sparkles size={12} />
                  {t('survey_list.btn_prompt')}
                </Link>
